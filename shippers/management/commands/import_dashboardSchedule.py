        ...
        while run < 3:
            ...
            if len(tomorrow_schedule) > 0:
                ...
                try:
                    ...
                    file_path = os.path.join(settings.BASE_DIR, '..\..\\teste.txt')
                ...
                if error == False:
                    ...
                    nr_entradas = (
                        0  # variavel para guardar o numero de linhas de schedule
                    )
                    ...
                    if len(dataXLSX) == 0:
                        dataXLSX = tomorrow.strftime("%Y-%m-%d")
                    ...
                    for idata in imported_data:  # CODIGO PARA CRIAR A SCHEDULE DAILY
                        ...
                        if ModelLinha_Config.objects.filter(
                            partnumber=xls_partnumber
                        ).exists():
                            mlc = ModelLinha_Config.objects.filter(
                                partnumber=xls_partnumber
                            ).last()
                            modelLinha = ModelLinha_Config.objects.filter(
                                end_item=mlc.end_item
                            )
                            for x in modelLinha:
                                boards[x.board] = 0
