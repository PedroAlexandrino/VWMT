    ...
    if request.method == "POST":
        if request.POST["password1"] == request.POST["password2"]:
            try:
                ...
                return render(
                    request, "accounts/sign.html", {"erro": "User already exists"}
                )
            ...
            return render(
                request, "accounts/sign.html", {"erro": "Passwords must match"}
            )
    else:
        ...
        if request.POST["password1"] == request.POST["password2"]:
            ...
            if user is not None:
                ...
                return render(
                    ...
                    {"erro": "Username or password incorrect"},
                )
        else:
            return render(
                request,
                "accounts/changePassword.html",
                {"erro": "Passwords must match"},
            )
    ...
    if request.method == "POST":
        ...
        if coordenador == "Nuno Pires":
            subject, from_email, to = (
                "New password requested for " + username + "",
                "noreply@visteon.com",
                ["npires2@visteon.com"],
            )
            text_content = (
                "This is an important automatic message \n\n"
                + "New password for "
                + username
                + " is  "
                + word
        ...
        if coordenador == "Daniel Leborgne":
            subject, from_email, to = (
                "New password automatic requested for " + username + "",
                "noreply@visteon.com",
