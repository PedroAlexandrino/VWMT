$(document).ready(function() {
    ...
    $('#scr-vtr-dynamic-print').DataTable({
        destroy: true,
        scrollY: '70vh',
        scrollCollapse: true,
        paging: false,
        ...
        buttons: [
            'copy', 'excel', 'pdf', 'print', 'searchPanes',
        ],
        columnDefs: [{
                searchPanes: {
                    show: true,
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            if (aData[2] == "2") {
                $('td', nRow).css('background-color', '#eef2d9');
            } else if (aData[2] == aData[3]) {
                $('td', nRow).css('background-color', '#dff0d8');
            } else if (aData[3] == "0") {
                $('td', nRow).css('background-color', '#fccfbc');
            }
        }
                ...
                'searchPanes', {
                    ...
                    action: function(e, dt, node, config) {
                        ...
                        $.each(rows_selected, function(index, rowId) {
                            ...
                            var rowSelected = { rowSelected: rowId };
                            $.ajax({
                                headers: { "X-CSRFToken": token },
                                url: "{% url 'vtrak:reposicao_material_selecionados' %}",
                                data: JSON.stringify(rowSelected),
                                type: 'post',
                                dataType: 'json',
                                success: function(data) {
                                    if (data.form_is_valid) {
                                        $('#scr-vtr-dynamic').DataTable().destroy();
    ...
    $('#scr-vtr-dynamic-data').DataTable({
        destroy: true,
        scrollY: '50vh',
        scrollCollapse: true,
        paging: false,
    ...
    $('#scr-vtr-dynamic-without-search').DataTable({
        destroy: true,
        scrollCollapse: true,
        paging: false,
        search: false,
